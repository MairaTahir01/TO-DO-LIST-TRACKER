    ...
    for task in tasks_to_show:
        ...
        with container:
            ...
            with cols[1]:
                ...
                if task["done"]:
                    st.markdown(f"<div class='task-item'><span class='task-text task-done'>{text_display}</span></div>", unsafe_allow_html=True)
                else:
                    # inline edit area
                    edit_expander = st.expander(f"{text_display}", expanded=False)
                    with edit_expander:
                        new_text = st.text_input("Edit task", value=text_display, key=f"edit_input_{task_id}", placeholder="Update your task here")
